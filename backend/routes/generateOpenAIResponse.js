...
async function generateOpenAIResponse(userId, message, chatType, relevantInfo, isAudio = false) {
    try {
        if (isAudio) {
            // Convert audio to text
            message = await speechToText(message);
        }
        ...
        if (isAudio) {
            ...
            return { text: responseText, audio: audioContent };
        }
